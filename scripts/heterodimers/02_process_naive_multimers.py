...
CHAI_OUTPUTS_DIRS = CHAI_OUTPUTS_DIR / 'naive_multimers' / 'naive_multimers_noXLs'
OUTPUT_CSV = PROCESSED_DATA_DIR / "03_naive_multimer_inter_xl_calculations.csv"
FAILED_CSV = PROCESSED_DATA_DIR / "03_naive_multimer_inter_xl_failed.csv"
...
def build_pair_dir_lookup(chai_outputs_dir):
    ...
    for d in chai_outputs_dir.iterdir():
        if not d.is_dir():
            continue
        match = re.match(r'pair_(\d+)_(TGRH88_\w+)_(TGRH88_\w+)$', d.name)
        if not match:
            continue
...
def main():
    ...
    pair_dir_lookup = build_pair_dir_lookup(CHAI_OUTPUTS_DIR)
