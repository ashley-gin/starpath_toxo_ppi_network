...
from config import REPO_ROOT, PROTEOME_CSV, PPI_CSV, CHAI_MULTIMER_DIR, INTER_XL_CSV, PROCESSED_DATA_DIR, CHAI_OUTPUTS_DIR #change to INTRA_XL_CSV to NULL_INTRA_XL_CSV if using null intra crosslinks
...
CHAI_OUTPUTS_DIRS = CHAI_OUTPUTS_DIR / 'xlrestraint_multimers' / 'xlrestraint_unconfident_A_OR_B"

OUTPUT_CSV = PROCESSED_DATA_DIR / "05_xlrestraint_multimer_unconfident_A_OR_B.csv" #UPDATE depending on which subset is being analyzed (e.g. A OR B, A AND B)
FAILED_CSV = PROCESSED_DATA_DIR / "05_xlrestraint_multimer_unconfident_A_OR_B_failed.csv"
...
# Columns written by process_ppi_directory plus legacy names from older XL variants.
# Stripped from subset inputs so prior run scores are not carried forward.
_CHAI_AGG_METRICS = frozenset(
    {
        # Canonical aggregate names
        "distance_mean",
        "distance_std",
        "distance_min",
        "distance_max",
        "intra_pae_protein_a_mean",
        "intra_pae_protein_b_mean",
        "ipae_interface_min_mean_across_models",
        "iptm_mean",
        "iptm_chai_default_mean",
        "ipae_interface_mean_best_model",
        "ipae_interface_mean_best_model_idx",
        "ipae_contact_mean_best_model",
        "ipae_contact_mean_best_model_idx",
        "ipae_contact_mean_best_model_distance",
        "ipae_xl_site_best_model",
        "ipae_xl_site_best_model_idx",
        "ipae_xl_site_best_model_distance",
        "ipae_xl_window_mean_best_model",
        "ipae_xl_window_mean_best_model_idx",
        "ipae_xl_window_mean_best_model_distance",
        "ipae_xl_window_median_best_model",
        "ipae_xl_window_median_best_model_idx",
        "ipae_xl_window_median_best_model_distance",
        # Prior names kept for cleanup compatibility
        "overall_chain_A_PAE",
        "overall_chain_B_PAE",
        "iPAE_min",
        "iPAE_XL_min",
        "min_iPAE_XL_model_idx",
        "iPAE_XL_min_distance",
        "iPAE_mean_min",
        "min_iPAE_mean_model_idx",
        "mean_contact_ipae_min",
        "min_mean_contact_ipae_model_idx",
        "min_mean_contact_ipae_distance",
        "iPAE_mean",
        "iPAE_1on2_mean",
        "iPAE_2on1_mean",
        "iptm_1on2_mean",
...
_MODEL_METRIC_SUFFIXES = (
    # Canonical per-model names
    "distance",
    "chains_swapped",
    "ipae_interface_min",
    "ipae_interface_mean",
...
CHAI_METRIC_COLUMNS = _CHAI_AGG_METRICS | frozenset(
    f"model_{i}_{s}" for i in range(5) for s in _MODEL_METRIC_SUFFIXES
)


def _identity_base_from_row(row) -> dict:
    """PPI / metadata columns only; drops prior Chai metric columns from subset rows."""
    return {k: row[k] for k in row.index if k not in CHAI_METRIC_COLUMNS}
...
def load_data():
    ...
    try:
        ...
        df_xlms = pd.read_csv(INTER_XL_CSV)
...
def expand_ppi_with_crosslinks(df_ppi, df_xlms):
    ...
    If a row already has Crosslink Position A/B (e.g. by_crosslink subset inputs),
    it is kept as a single row — no join against all XL-MS rows for that pair.
    """
    expanded_rows = []
    skipped_prefilled = 0
    ...
    for _, ppi_row in df_ppi.iterrows():
        ...
        pos_a = ppi_row.get("Crosslink Position A")
        pos_b = ppi_row.get("Crosslink Position B")
        if pd.notna(pos_a) and pd.notna(pos_b):
            expanded_rows.append(ppi_row.to_dict())
            skipped_prefilled += 1
            continue
    ...
    if skipped_prefilled:
        logger.info(
            f"{skipped_prefilled} PPI row(s) already had crosslink positions — "
            f"kept one row each (no XL-MS expansion per protein pair)"
        )
...
def make_xl_id(residue_a, pos_a, residue_b, pos_b):
    """
    Build crosslink id string matching single-inter-XL directory suffixes, e.g. K363_K124.
    """
...
def build_pair_dir_lookup(chai_outputs_dirs):
    """
    Build (protein_a, protein_b, xl_id) -> (dir, status) lookup from directories like:
    pair_0_TGRH88_000140_TGRH88_035710_K363_K124

    Parameters
    ----------
    chai_outputs_dirs : pathlib.Path or sequence of Path
        Root folder(s) that each contain pair_* subdirectories. Multiple roots are merged;
        if the same (prot_a, prot_b, xl_id) appears in more than one root, the entry with
        the lowest pair_* index is kept (same rule as within a single root).
    """
    if isinstance(chai_outputs_dirs, (str, Path)):
        roots = [Path(chai_outputs_dirs)]
    else:
        roots = [Path(r) for r in chai_outputs_dirs]

    dir_re = re.compile(r"^pair_(\d+)_(TGRH88_\d+)_(TGRH88_\d+)_(.+)$")
    candidates = {}  # (prot_a, prot_b, xl_id) -> (pair_idx, dir)

    for root in roots:
        if not root.is_dir():
            logger.warning(f"CHAI outputs path missing or not a directory: {root}")
            continue
        for d in root.iterdir():
            if not d.is_dir():
                continue
            match = dir_re.match(d.name)
            if not match:
                continue

            pair_idx = int(match.group(1))
            prot_a, prot_b = match.group(2), match.group(3)
            xl_id = match.group(4)
    ...
    lookup = {}
    for (prot_a, prot_b, xl_id), (_, d) in candidates.items():
        ...
        if len(cif_files) != 5 or len(npz_files) != 5:
            lookup[(prot_a, prot_b, xl_id)] = (d, "incomplete_files")
        else:
            lookup[(prot_a, prot_b, xl_id)] = (d, "valid")

    n_valid = sum(1 for _, s in lookup.values() if s == "valid")
    logger.info(f"Found {len(lookup)} crosslink-specific pair directories ({n_valid} valid)")
    return lookup


def get_pair_dir_for_row(pair_dir_lookup, row):
    """
    Match one expanded CSV row to its exact crosslink-specific directory.
    Returns (ppi_dir, status, matched_xl_id) or (None, reason, None).
    """
    protein_a = row["Protein A"]
    protein_b = row["Protein B"]
    pos_a = row.get("Crosslink Position A")
    pos_b = row.get("Crosslink Position B")
    res_a = row.get("Crosslinked Residue A")
    res_b = row.get("Crosslinked Residue B")

    if pd.isna(pos_a) or pd.isna(pos_b):
        return None, "missing_crosslink_positions", None

    xl_id = make_xl_id(res_a, pos_a, res_b, pos_b)
    xl_id_swapped = make_xl_id(res_b, pos_b, res_a, pos_a)

    dir_entry = pair_dir_lookup.get((protein_a, protein_b, xl_id))
    if dir_entry is None:
        dir_entry = pair_dir_lookup.get((protein_b, protein_a, xl_id_swapped))

    if dir_entry is None:
        return None, "no_directory_found", None

    ppi_dir, status = dir_entry
    return ppi_dir, status, xl_id
...
def get_chain_assignment(coords_dict, len_protein1, len_protein2):
    """
    Determine whether the CIF chains are in expected order (A=protein A, B=protein B)
    or swapped (A=protein B, B=protein A) by comparing residue counts to len_protein1.

    Returns
    -------
    swapped : bool
        True if the CIF has chains in reverse order relative to the CSV.
    chain_a_id : str
        The CIF chain label that physically contains protein A's residues.
    chain_b_id : str
        The CIF chain label that physically contains protein B's residues.
    """
    ...
    if match_A_to_protA and match_B_to_protB and not (match_A_to_protB and match_B_to_protA):
        return False, "A", "B"  # unambiguous, no swap
    elif match_A_to_protB and match_B_to_protA and not (match_A_to_protA and match_B_to_protB):
        return True, "B", "A"   # unambiguous swap
    else:
        raise ValueError(
...
def process_ppi_directory(ppi_dir, xl_pos_a, xl_pos_b, len_protein1, len_protein2):
    ...
    Same analysis as naive-window canonical naming script.
    ...
    for idx in shared_idx:
        ...
        try:
            ...
            swapped, chain_a_id, chain_b_id = get_chain_assignment(coords, len_protein1, len_protein2)
            ...
            ipae_xl, _ipae_xl_1on2, _ipae_xl_2on1 = compute_ipae_at_crosslink(
                pae,
                xl_pos_a_eff,
                xl_pos_b_eff,
            ...
            iptm_mean, _iptm_1on2, _iptm_2on1 = compute_iptm_from_chain_pair(
            ...
            iptm_chai = (
                float(npz_data['iptm'].item())
                if hasattr(npz_data['iptm'], 'item')
                else float(npz_data['iptm'])
            )
...
def save_checkpoint(valid_rows, failed_rows, results_dir):
    ...
    checkpoint_output = results_dir / "checkpoint_XL_chai_processing_ipae_window_20260417.csv"
    checkpoint_failed = results_dir / "checkpoint_failed_XL_chai_processing_ipae_window_20260417.csv"
...
def main():
    ...
    logger.info("Starting PPI crosslink analysis pipeline (XL chai processing, canonical naming + iPAE window)")

    df_ppi, df_proteome, df_xlms = load_data()
    _drop_metrics = [c for c in df_ppi.columns if c in CHAI_METRIC_COLUMNS]
    if _drop_metrics:
        df_ppi = df_ppi.drop(columns=_drop_metrics)
        logger.info(f"Dropped {len(_drop_metrics)} prior Chai metric column(s) from input PPI table")

    proteome_mapping = create_proteome_mapping(df_proteome)
    pair_dir_lookup = build_pair_dir_lookup(CHAI_OUTPUTS_DIRS)
    df_expanded = expand_ppi_with_crosslinks(df_ppi, df_xlms)
    ...
    for idx, row in df_expanded.iterrows():
        ...
        ppi_dir, status, matched_xl_id = get_pair_dir_for_row(pair_dir_lookup, row)
        if ppi_dir is None:
            logger.warning(f"Skipping {protein_a} - {protein_b}: {status}")
            failed_row = _identity_base_from_row(row)
        ...
        # Optional strict suffix check (highly recommended)
        xl_id_swapped = make_xl_id(
            row.get("Crosslinked Residue B"), row.get("Crosslink Position B"),
            row.get("Crosslinked Residue A"), row.get("Crosslink Position A"),
        )
        dir_xl_suffix = "_".join(ppi_dir.name.split("_")[-2:])
        if matched_xl_id and dir_xl_suffix not in (matched_xl_id, xl_id_swapped):
            logger.warning(
                f"Crosslink mismatch: row xl_id={matched_xl_id} (or swapped {xl_id_swapped}), "
                f"dir has {dir_xl_suffix}; skipping"
            )
            failed_row = _identity_base_from_row(row)
            failed_row["failure_reason"] = "crosslink_id_mismatch"
            failed_rows.append(failed_row)
            continue
        ...
        try:
            ...
            output_row = _identity_base_from_row(row)
