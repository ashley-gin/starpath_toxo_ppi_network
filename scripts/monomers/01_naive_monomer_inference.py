...
"""
Process a single protein monomer through Chai-1.
...
from config import REPO_ROOT, PROTEOME_CSV, CHAI_MONOMER_DIR
...
def mapping(df_proteome):
    """Create mapping from gene name to sequence"""
    mapping_by_gene = {}
    ...
    for _, row in df_proteome.iterrows():
        ...
        if pd.isna(sequence) or len(sequence.strip()) == 0:
            logger.warning(f"Empty sequence for protein entry: {entry}")
            continue
        
        for gene in gene_names.split():
            if gene.startswith("TGRH88"): 
                if gene not in mapping_by_gene:
                    mapping_by_gene[gene] = {
                        "uniprot_id": entry,
                        "sequence": sequence,
                        "all_gene_names": [g for g in gene_names.split() if g.startswith("TGRH88")]
                    }
                else:
                    if mapping_by_gene[gene]["uniprot_id"] != entry:
                        logger.warning(f"Conflict: {gene} maps to both {mapping_by_gene[gene]['uniprot_id']} and {entry}")
                        
    return mapping_by_gene

def check_if_monomer_complete(output_dir: Path, protein: str, idx: int) -> bool:
    """Check if protein monomer already has complete predictions"""
...
def process_single_monomer(idx, protein, base_path):
    """Process a single protein monomer through Chai-1"""

    # Check if already complete
    chai_output_dir = CHAI_MONOMER_DIR
    if check_if_monomer_complete(chai_output_dir, protein, idx):
        logger.info(f"Monomer {idx} ({protein}) already complete - skipping")
    ...
    # Setup directories
    chai_fasta_dir = Path(CHAI_MONOMER_DIR / 'chai_fasta_monomers' )
    ...
    try:
        with open(temp_fasta_path, 'w') as fasta_file:
            fasta_file.write(f">protein|{protein}\n{seq['sequence']}\n")
            
        logger.info(f"Created FASTA for {protein}")
        
        # Run Chai-1 inference
        monomer_output_dir = chai_output_dir / f"monomer_{idx}_{protein}"
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python 01_naive_monomer_inference.py <idx> <protein>")
        sys.exit(1)
    ...
    logger.info(f"Processing monomer {idx}: {protein}")
    
    try:
        process_single_monomer(idx, protein, base)
        sys.exit(0)  # Success
    except Exception as e:
